    ...
    while flag:
        ...
        for x in range(len(s.body)):
            if s.body[x].pos in list(map(lambda z: z.pos, s.body[x + 1:])):
                prevScore = read_json(folder + 'highscore.json')['highscore']
                highscore = str(len(s.body))
                if int(highscore) > int(prevScore):
                    write_json(folder + 'highscore.json', {"highscore":highscore})
                    print('New highscore: ' + highscore)
                    message_box('You Lost!', 'New highscore: ' + highscore + '\nPlay again')
                else:
                    print('Score: ' + highscore)
                    message_box('You Lost!', 'Score: ' + highscore + '\nPlay again')
                s.reset((10, 10))
                break
